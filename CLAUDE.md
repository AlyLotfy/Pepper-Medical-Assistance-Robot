...
- **Python 3 (laptop-side)**: Flask backend, Whisper STT, Claude/Ollama LLM, FAISS RAG, SQLite, WebSocket bridge. Lives under `Pepper-Controller-main-2/pepper_ui/server/` and `Pepper-Controller-main-2/pepper_voice/ws_bridge.py`.
...
`app.py` has a unified `call_llm()` (around line 1513) that routes to either Claude API (online) or Ollama (offline) based on `OFFLINE_MODE` env var. Both paths use the **same agentic tool-calling loop** — tools are defined once and work for both providers. When adding a new AI feature or tool, implement it once in this loop, don't branch.

- Online: Anthropic Claude (`claude-haiku-4-5-20251001`), needs `CLAUDE_API_KEY` / `ANTHROPIC_API_KEY` in `.env`.
...
# Full run — robot + server, Claude API
python main.py
