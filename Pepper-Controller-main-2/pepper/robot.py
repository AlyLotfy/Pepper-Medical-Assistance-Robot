...
class Pepper:
    ...
    **Real robot controller**
    ...
    def listen(self, lang):
        ...
        remote_filename = "speech_{}.wav".format(unique_id)
        remote_path = "/home/nao/{}".format(remote_filename)
        self.speech_service.setAudioExpression(False)
        self.speech_service.setVisualExpression(False)
        ...
        while True:
            ...
            if self.memory_service.getData("ALSpeechRecognition/Status") == "SpeechDetected":
                self.audio_recorder.startMicrophonesRecording(remote_path, "wav", 48000, (0, 0, 1, 0))
                print("[INFO]: Robot is listening to you")
        ...
        self.download_file(remote_filename)
        self.speech_service.setAudioExpression(True)
        self.speech_service.setVisualExpression(True)

        return self.speech_to_text(remote_filename, lang)
    ...
    def recognize_google(self, lang):
        ...
        :param lang: str
        """
        local_filename = self.recordSound()
        return self.speech_to_text(local_filename, lang)

    def recordSound(self):
        """Record audio to a unique filename to avoid race conditions when called concurrently."""
        unique_id = uuid.uuid4().hex[:8]
        remote_filename = "speech_{}.wav".format(unique_id)
        remote_path = "/home/nao/{}".format(remote_filename)
        self.audio_recorder.stopMicrophonesRecording()
