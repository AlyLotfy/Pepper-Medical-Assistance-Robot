...
class ConversationMemory:
    ...
    def _call_claude(self, prompt, max_tokens=300):
        try:
            resp = requests.post(
                self.url,
    ...
    def summarize_and_save(self, patient_id, conversation_history):
        ...
        """
        if not patient_id or not conversation_history or not self.api_key:
            return
        ...
        summary = self._call_claude(prompt, max_tokens=200)
        if not summary:
            return
        ...
        facts_text = self._call_claude(facts_prompt, max_tokens=200)
        try:
