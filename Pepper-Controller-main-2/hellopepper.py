...
if __name__ == "__main__":
    ...
    try:
        with open(_conf_path) as _f:
            _conf = yaml.safe_load(_f)
        ip_address = _conf["configuration"]["default_ip"]
        port = int(_conf["configuration"]["default_port"])
    except Exception as _e:
        print("[WARN]: Could not read conf.yaml ({}), falling back to defaults.".format(_e))
